...
<div class="table-responsive rounded-2 mb-4">
    ...
    <table class="table border text-nowrap customize-table mb-3 align-middle">
      ...
      <tbody>
        @forelse ($courseForThose as $key => $item)
        <tr>
            <td>
                <span>{{ $courseForThose->firstItem()+$key }}</span>
            </td>
            <td class="text-center">
                <span>{{ $item->content }}</span>
            </td>
            <td class="text-center">
                <img src="{{ asset('storage/courseThose/') }}/{{ $item->image }}" alt="" width="60" height="60">
            </td>
            <td class="text-end">
                <div class="dropdown dropstart">
                    ...
                    <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton" style="">
                        ...
                        <li>
                        <a class="dropdown-item d-flex align-items-center gap-3" href="{{ route('courseForThoseDelete',$item->id) }}"><i class="fs-4 ti ti-trash"></i>Delete</a>
                        </li>
                    </ul>
                </div>
            </td>
            ...
            <div class="modal-dialog modal-dialog-centered" role="document">
              <div class="modal-content border-0">
                <div class="modal-header bg-primary">
                  <h6 class="modal-title text-white">Edit CourseForThose</h6>
                  <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form action="{{ route('courseForThoseEdit') }}" method="POST" id="addnotesmodalTitle" enctype="multipart/form-data">
                    @csrf
                                    ...
                                    <div class="col-md-12 mb-3">
                                        ...
                                        <div class="note-title">
                                            ...
                                            @error('image','update')
                                                <div class="alert alert-danger">{{ $message }}</div>
                                            @enderror
                                            <img src="{{ asset('storage/courseThose/') }}/{{ $item->image }}" alt="Image Preview" width="80"  id="imagePreview{{ $item->id }}">
                                        </div>
                                        <script>
    ...
    {{ $courseForThose->links() }}
    <div class="modal" id="addData">
        <div class="modal-dialog modal-dialog-centered" role="document">
          <div class="modal-content border-0">
            <div class="modal-header bg-primary">
              <h6 class="modal-title text-white">Add CourseForThose</h6>
              <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="{{ route('courseForThosePost',$courseid) }}" method="POST" id="addnotesmodalTitle" enctype="multipart/form-data">
                @csrf
                <div class="modal-body">
                    <div class="notes-box">
                        <div class="notes-content">
