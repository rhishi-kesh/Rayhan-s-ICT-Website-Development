...
@section('content')
    <div class="table-responsive rounded-2 mb-4">
                        ...
                        @foreach ($seminer as $key => $data)
                            <div class="tab-pane {{ $key == 0 ? ' active' : ''  }}" id="data{{ $data->id }}" role="tabpanel">
                                <table class="table border text-nowrap customize-table mb-0 align-middle">
                                    ...
                                    <tbody>
                                        @forelse ($seminerRegister->where('seminer_id', $data->id) as $key=> $item)
                                            <tr>
                                                <td>
                                                    <span> {{ $item->id }} </span>
                                                </td>
                                                ...
                                                <td class="text-end">
                                                    <div class="dropdown dropstart text-center">
                                                        ...
                                                        <a href="{{ route('seminarDataDelete', $item->id) }}" class="text-muted pe-2"
                                                            aria-expanded="false">
        ...
        {{ $seminerRegister->withQueryString()->links() }}
